...
export class PaymentService {
  ...
  async create(payment: Payment): Promise<{ payment: Payment }> {
    ...
    if (!['PREMIUN', 'VIP', 'BASIC'].includes(payment.name))
      throw new Error('the name must be PREMIUN or VIP or BASIC');
    payment.active = true;
    ...
    if (savedPayment.user) {
      ...
      if (user) {
        ...
        user.has_premium_service = activePayments.some(
          (p) => p.name === 'PREMIUN',
        );
  ...
  async update(id: number, payment: Payment): Promise<Payment> {
    ...
    if (!result) throw new NotFoundException('Payment not found');
    if (payment.price <= 0) throw new Error('the price must be greater than 0');
    if (!['PREMIUN', 'VIP'].includes(payment.name))
      throw new Error('the name must be PREMIUN or VIP');
  ...
  async findByPremiumService(userId: number): Promise<Payment[]> {
    ...
    const payments = user.payments.filter(
      (payment) => payment.name === 'PREMIUN' && payment.active,
    );
    if (payments.length === 0)
      throw new NotFoundException(
        'No active premium services found for this user',
      );
    return payments;
  }
  ...
  async findByVipService(userId: number): Promise<Payment[]> {
    ...
    if (!user) throw new NotFoundException('User not found');
    const payments = user.payments.filter(
      (payment) => payment.name === 'VIP' && payment.active,
    );
    if (payments.length === 0)
      throw new NotFoundException('No active VIP services found for this user');
    return payments;
  }
