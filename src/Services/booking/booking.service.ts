...
export class BookingService {
  ...
  async create(data: Reservation): Promise<Reservation> {
    ...
    if (new Date(data.check_in) >= new Date(data.check_out))
      throw new ConflictException(' Check in must be greater than check out');
...
WITH
hotel_base_info AS (
    -- Calcula estadísticas base para el hotel específico (sin filtro de fecha general, pero ahora parametrizado por hotel)
    SELECT
        h.id AS hotel_id,
    ...
    LEFT JOIN reservation res_hist ON rm.id = res_hist.room_id -- Considera todas las reservas para este hotel
    WHERE h.id = $1 -- <<< PARÁMETRO $1 UTILIZADO AQUÍ
    GROUP BY h.id
),
...
reservation_client_metrics AS (
    SELECT
        ...
        -- >>> CÁLCULO DE total_booking_value MOVIDO AQUÍ PARA REUTILIZACIÓN <<<
        (rm.price::DOUBLE PRECISION * GREATEST(1, DATE_PART('day', res.check_out::timestamp - res.check_in::timestamp)))::DOUBLE PRECISION AS total_booking_value,
        c.birth_date,
        c.rol,
    ...
    WHERE rm.hotel_id = $1 -- <<< PARÁMETRO $1 UTILIZADO AQUÍ
      AND EXTRACT(MONTH FROM res.check_in) = $2 -- <<< PARÁMETRO $2 UTILIZADO Y DESCOMENTADO
      AND c.rol = 'user'
...
reservation_context_calc AS (
    ...
    WHERE r_res.hotel_id = $1 -- <<< PARÁMETRO $1 UTILIZADO AQUÍ
      AND EXTRACT(MONTH FROM res.check_in) = $2 -- <<< PARÁMETRO $2 UTILIZADO Y DESCOMENTADO (sobre la reserva principal 'res')
      AND res.check_in IS NOT NULL
      AND res.check_out IS NOT NULL
      AND res.check_out > res.check_in
      AND res.created_at <= res.check_in
...
-- ========= SECCIÓN FINAL CON CASTS EXPLÍCITOS Y LÓGICA DE PAGO MEJORADA =========
SELECT
    -- Identificadores
    rcm.reservation_id,
    rcm.client_id,
    ...
    CASE
        ...
        ELSE 'frequent' -- Ajustado para incluir > 10 y diferenciar de 'regular'
    END AS client_type,
    ...
    CASE
        ...
        WHEN rcm.client_avg_historical_price IS NULL THEN 'new_client' -- Podría ser redundante con el anterior
        WHEN rcm.client_avg_historical_price < COALESCE(hbi.avg_room_price_hotel, 0)::DOUBLE PRECISION THEN 'below_avg_spender'
        WHEN rcm.client_avg_historical_price < COALESCE(hbi.avg_room_price_hotel, 0)::DOUBLE PRECISION * 1.3 THEN 'avg_spender'
        ELSE 'above_avg_spender'
    ...
    CASE
        ...
        WHEN COALESCE(hbi.total_reservations_hotel_overall, 0) < 50 THEN 'medium_volume' -- Ajustado
        ELSE 'high_volume'
    ...
    rcm.total_booking_value, -- <<< UTILIZANDO EL VALOR CALCULADO EN RCM
    CASE
        WHEN DATE_PART('day', rcm.check_in::timestamp - rcm.created_at::timestamp) < 3 THEN 'lead_0_2_days'
        WHEN DATE_PART('day', rcm.check_in::timestamp - rcm.created_at::timestamp) < 7 THEN 'lead_3_6_days'
        WHEN DATE_PART('day', rcm.check_in::timestamp - rcm.created_at::timestamp) < 30 THEN 'lead_7_29_days'
        ELSE 'lead_30_plus_days'
    END AS booking_lead_time_category, -- Renombrado para claridad
    DATE_PART('day', rcm.check_in::timestamp - rcm.created_at::timestamp)::INTEGER AS booking_lead_time_days, -- Métrica numérica también
    CASE WHEN EXTRACT(MONTH FROM rcm.check_in) IN (6,7,8,12) THEN 1 ELSE 0 END AS is_peak_season, -- Esto será más constante si se filtra por mes

    -- Métricas de Contexto/Concurrencia
    ...
    CASE
        WHEN COALESCE(rcc.concurrent_total, 0) = 0 THEN 'demand_none'
        WHEN COALESCE(rcc.concurrent_total, 0) < 5 THEN 'demand_low'
        WHEN COALESCE(rcc.concurrent_total, 0) < 15 THEN 'demand_medium'
        ELSE 'demand_high'
    END AS concurrent_demand_level,

    -- >>> LÓGICA DE PAGO MEJORADA Y MÁS DETALLADA <<<
    CASE
        WHEN pr.reservation_id IS NULL THEN 'P_NoPaymentRecord' -- No hay fila en payment_reservation
        WHEN pr.amount IS NULL THEN 'P_PaymentRecordExists_AmountIsNull' -- Hay fila, pero el monto es NULL (si el esquema lo permite)
        WHEN pr.amount = 0 THEN 'P_ZeroAmountRecorded'
        WHEN pr.amount > 0 AND pr.amount < (rcm.total_booking_value * 0.50) THEN 'P_Paid_LessThan50Pct'
        WHEN pr.amount >= (rcm.total_booking_value * 0.50) AND pr.amount < rcm.total_booking_value THEN 'P_Paid_50To99Pct'
        WHEN pr.amount >= rcm.total_booking_value THEN 'P_Paid_FullOrMore'
        ELSE 'P_PaymentStatus_Inconsistent' -- Para casos inesperados, como montos negativos si no están restringidos
